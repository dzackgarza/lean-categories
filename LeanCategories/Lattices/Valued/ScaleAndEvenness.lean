...
def scaleIdeal (L : IntegralLatticeCat R) : Ideal R :=
  LinearMap.range L.obj.form

/-- The norm ideal generated by the diagonal values `b(x,x)`. -/
def normIdeal (L : IntegralLatticeCat R) : Ideal R :=
  Ideal.span (Set.range (quadraticMap L))

/-- The quotient in the defining exact sequence for the scale ideal. -/
abbrev ScaleCokernel (L : IntegralLatticeCat R) := R ⧸ scaleIdeal L

/-- The quotient in the defining exact sequence for the value ideal. -/
abbrev ValueCokernel (L : IntegralLatticeCat R) := R ⧸ normIdeal L

def scaleInclusion (L : IntegralLatticeCat R) : scaleIdeal L →ₗ[R] R :=
  (scaleIdeal L).subtype

def scaleProjection (L : IntegralLatticeCat R) : R →ₗ[R] ScaleCokernel L :=
  Submodule.mkQ (scaleIdeal L)

def valueInclusion (L : IntegralLatticeCat R) : normIdeal L →ₗ[R] R :=
  (normIdeal L).subtype

def valueProjection (L : IntegralLatticeCat R) : R →ₗ[R] ValueCokernel L :=
  Submodule.mkQ (normIdeal L)
...
theorem exact_scaleIdeal (L : IntegralLatticeCat R) :
    Function.Exact (scaleInclusion L) (scaleProjection L) :=
  LinearMap.exact_subtype_mkQ (scaleIdeal L)

theorem exact_normIdeal (L : IntegralLatticeCat R) :
    Function.Exact (valueInclusion L) (valueProjection L) :=
  LinearMap.exact_subtype_mkQ (normIdeal L)
...
theorem isIIntegral_iff_scaleIdeal_le
    (L : IntegralLatticeCat R) (I : Ideal R) :
    IsIIntegral L I ↔ scaleIdeal L ≤ I := by
...
theorem isIEven_iff_normIdeal_le
    (L : IntegralLatticeCat R) (I : Ideal R) :
    IsIEven L I ↔ normIdeal L ≤ I := by
