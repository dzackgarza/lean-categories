...
/-!
The classical level of an even integral lattice is controlled by its quadratic
discriminant form.  This is stronger than the exponent of the discriminant
group: the latter only sees the bilinear discriminant module.
-/

/-- The `ℤ`-submodule generated by the values of the quadratic discriminant form. -/
noncomputable def discriminantQuadraticValueModule
    (L : IntegralLatticeCat ℤ)
    (hL : IsFractionFieldPerfect ℤ L) (hEven : IsEven L) :
    Submodule ℤ (DoubleFractionValueQuotient ℤ) :=
  Submodule.span ℤ (Set.range (evenDiscriminantQuadraticMap L hL hEven))

/-- The ideal of integers that annihilate the quadratic discriminant form. -/
noncomputable def levelIdeal
    (L : IntegralLatticeCat ℤ) [Module.Finite ℤ L.obj.carrier]
    (hL : IsFractionFieldPerfect ℤ L) (hEven : IsEven L) : Ideal ℤ :=
  Module.annihilator ℤ (discriminantQuadraticValueModule L hL hEven)

/-- The standard level of a finite even integral lattice.

It is the positive generator, up to sign, of the annihilator ideal of the
quadratic discriminant values.  Thus it records the least positive `N` for
which `N q_A` is zero in `Frac(ℤ) / 2ℤ`, rather than the exponent of `A_L`.
-/
