...
abbrev algebraicClosure : AlgebraCat K := AlgCat.of K (AlgebraicClosure K)

/-- The canonical structure morphism into `AlgebraicClosure K`. -/
abbrev algebraicClosureMap : AlgCat.of K K ⟶ algebraicClosure K :=
  AlgCat.ofHom (Algebra.ofId K (AlgebraicClosure K))
...
abbrev separableClosure : AlgebraCat K := AlgCat.of K (SeparableClosure K)

/-- The canonical structure morphism into `SeparableClosure K`. -/
abbrev separableClosureMap : AlgCat.of K K ⟶ separableClosure K :=
  AlgCat.ofHom (Algebra.ofId K (SeparableClosure K))
...
abbrev splittingField (f : Polynomial K) : AlgebraCat K := AlgCat.of K f.SplittingField

/-- The canonical structure morphism into the splitting field of `f`. -/
abbrev splittingFieldMap (f : Polynomial K) : AlgCat.of K K ⟶ splittingField K f :=
  AlgCat.ofHom (Algebra.ofId K f.SplittingField)
...
abbrev tensorProduct : AlgebraCat R := AlgCat.of R (TensorProduct R A B)

/-! The canonical left algebra morphism into the tensor product algebra. -/
abbrev tensorProductIncludeLeft : AlgCat.of R A ⟶ tensorProduct R A B :=
  AlgCat.ofHom Algebra.TensorProduct.includeLeft

/-! The canonical right algebra morphism into the tensor product algebra. -/
abbrev tensorProductIncludeRight : AlgCat.of R B ⟶ tensorProduct R A B :=
...
abbrev groupRing : AlgCat.{max u v, u} R := AlgCat.of R (MonoidAlgebra R G)

/-! The canonical coefficient embedding `R ⟶ R[G]`. -/
abbrev groupRingCoefficient :
    R →ₐ[R] MonoidAlgebra R G :=
  MonoidAlgebra.singleOneAlgHom (R := R) (A := R) (M := G)

end GroupRing

/-- The multivariate polynomial algebra over `R`, with variables indexed by `σ`. -/
abbrev mvPolynomial (σ : Type v) : AlgebraCat R := AlgCat.of R (MvPolynomial σ R)

/-- The formal power-series algebra over `R`. -/
abbrev powerSeries : AlgebraCat R := AlgCat.of R (PowerSeries R)

/-- The multivariate formal power-series algebra over `R`, with variables indexed by `σ`. -/
abbrev mvPowerSeries (σ : Type v) : AlgebraCat R := AlgCat.of R (MvPowerSeries σ R)
...
abbrev laurentSeries : AlgebraCat R := AlgCat.of R (LaurentSeries R)

/-- The localization of `R` at a submonoid, as an `R`-algebra. -/
abbrev localization (M : Submonoid R) : AlgebraCat R := AlgCat.of R (Localization M)
...
abbrev localizationAway (r : R) : AlgebraCat R :=
  AlgCat.of R (Localization.Away r)
...
abbrev localizationAwayMap (r : R) : R →ₐ[R] (localizationAway R r).carrier :=
  Algebra.ofId R (Localization.Away r)
...
abbrev localizationAtPrime (P : Ideal R) [P.IsPrime] : AlgebraCat R :=
  AlgCat.of R (Localization.AtPrime P)
...
abbrev localizationAtPrimeMap (P : Ideal R) [P.IsPrime] :
  ...
  Algebra.ofId R (Localization.AtPrime P)
