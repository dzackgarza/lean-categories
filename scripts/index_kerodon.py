#!/usr/bin/env python3
"""Build and verify the FC17 Kerodon stable-tag source catalogue.
...
4. writes a normalized JSON snapshot and the canonical Sweep-I FC17 catalogue.
...
DEFAULT_OUTPUT = (
    REPOSITORY_ROOT / "corpus" / "foundational-corpus-units-fc17-kerodon.md"
)
...
def source_unit_id(tag: str) -> str:
    """Return the stable FC17 source-unit ID determined by a Kerodon tag.

    The numerical suffix is the one-based allocator ordinal of the permanent
    Kerodon tag, not the item's current chapter/reference.  Moving an item in
    the live text therefore cannot renumber the corresponding corpus unit.
    """

    return f"FC17-KER-U{tag_number(tag) + 1:05d}"
...
def render_markdown(snapshot: Mapping[str, Any]) -> str:
    """Render the canonical Sweep-I FC17 catalogue."""
    ...
    lines = [
        "---",
        "type: reference",
        "title: Foundational corpus units FC17 Kerodon",
        "description: '# Foundational corpus units FC17 Kerodon'",
        "tags:",
        "- project",
        "- reference",
        ...
        "",
        "This is the canonical Sweep-I catalogue for the corpus-v3 FC17 source. "
        "The admitted scope is every non-structural stable Kerodon tag present in "
        "the pinned snapshot, including the retired part and exercises. Statement "
        "and proof prose remain at Kerodon; this repository stores stable tags, "
        "kinds, printed references, explicit names, hierarchy, and current/retired "
        "state. Sweep II must read the authoritative tagged statement before "
        "accepting any implementation mapping.",
        "",
        ...
        "- FC17 membership is the pinned snapshot above. A refresh that changes "
        "the tag population is a corpus-scope change and requires an explicit "
        "corpus amendment before the generated catalogue can become authoritative.",
        "",
        "## Unit counts",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            "Each `FC17-KER-Uxxxxx` ID is deterministically derived from the "
            "one-based allocator ordinal of Kerodon's permanent tag. The ID is "
            "therefore stable even if Kerodon moves the item to another section.",
            "",
        ]
    )
    ...
    for chapter in chapters:
        ...
        lines.extend(
            [
                ...
                "",
                "| ID | Kerodon tag | Reference | Kind | Explicit name | Section context | State | Source statement | Dependencies |",
                "| --- | --- | --- | --- | --- | --- | --- | --- | --- |",
            ]
        )
        for record in chapter_units:
            ...
            unit_id = source_unit_id(tag)
            lines.append(
                f"| `{unit_id}` | [{tag}]({BASE_URL}/tag/{tag}) | "
                f"{markdown_escape(str(record['reference']))} | "
                f"{title_case_kind(str(record['kind']))} | {name} | {context} | "
                f"{record['state']} | "
                f"[statement]({BASE_URL}/data/tag/{tag}/content/statement) | — |"
            )
        lines.append("")
