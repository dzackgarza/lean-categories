...
Sweep III C05-U070 is realized by `LeanCategories/Homological/ExactCoupleSpectralSequence.lean`. `BigradedExactCouple` supplies degree-shifting `D,E,i,j,k` families, with shifts represented by index automorphisms and exactness at every bidegree; `page` forms the intrinsic `k≫j` homological complex. `DerivedData` records only the derived-object formulas `D'=im(i)` and `E'=H(E,kj)`. `DerivedTower` records an actual rooted sequence of successive derived exact couples, and its `spectralSequence` constructor computes Mathlib's pages and successor homology isomorphisms without requiring a global derivation choice for unrelated exact couples or accepting page/differential data as input. The focused module build succeeds, and temporary probes verify the allowed page differential is literally `k≫j`, off-shape differentials vanish, and the starting page is the original `E` family. The source's closed differential-bidegree formula and morphism functoriality remain Sweep IV.

Sweep III C05-U077 is realized by `LeanCategories/Homological/BocksteinSpectralSequence.lean`. The long exact sequence constructs `bocksteinDifferential = k≫j` and `bocksteinComplex`; `BocksteinDerived.derived` then constructs `D'=im(ℓ)` and `E'=H(E,β)`, including all three derived exactness proofs, so `BocksteinDerived.iterate` is a genuine recursive exact-couple tower. A vertical artificial bigrading with inert first coordinate produces `BigradedExactCouple` terms; explicit short-complex isomorphisms identify their page homology with the singly graded Bockstein homology, giving `bigradedDerivedData` and `bigradedTower`. The resulting `BocksteinDerived.spectralSequence` is therefore obtained from U070 rather than supplied. Focused source/build acceptance succeeds, and semantic probes verify page zero is the original `E`, its differential is β, page one is page-zero homology, and the successor isomorphism is the derived-couple homology identification. Proposition 5.9.10 and the standard regrading/differential-degree formulas remain theorem work.
