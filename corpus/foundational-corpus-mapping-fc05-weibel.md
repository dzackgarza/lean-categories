...
Sweep III C05-U070 is realized by `LeanCategories/Homological/ExactCoupleSpectralSequence.lean`. `BigradedExactCouple` supplies degree-shifting `D,E,i,j,k` families with exactness at every bidegree; `page` forms the intrinsic `k≫j` homological complex. `DerivedData` records the derived-object formulas `D'=im(i)` and `E'=H(E,kj)`, and a supplied one-step derivation operation is iterated by `iterate`; `spectralSequence` then constructs Mathlib's page family and successor homology isomorphisms without accepting page or differential data as input. The focused module build succeeds, and temporary probes verify the allowed page differential is literally `k≫j`, off-shape differentials vanish, and page `a` has the original `E` objects. The source's closed differential-bidegree formula and morphism functoriality remain Sweep IV.
