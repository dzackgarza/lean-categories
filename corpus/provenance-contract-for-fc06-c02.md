...
- **Superseded 2026-09-13:** the 2026-09-06 whole-row/strict-bundle rule was rejected. Mapping is clause-wise: existing declarations own the clauses they actually provide, and the comparison assembling them is recorded explicitly. A row remains `unmatched` only for the specific residual theorem/bridge not supplied after the full search order.
...
- Historical 2026-09-06 baseline: reconciliation changed only nine route rows and reported `mathlib` 82 / `unmatched` 243. Those counts are not current and must not be used as a completion measure; `FOUNDATIONAL_FRONTIER.md` is regenerated from the tracked clause-wise mappings.

- 2026-09-13 remap-strict-bundle, sheaf block U001–U017: the old whole-row rule was removed clause-by-clause. `TopCat.Presheaf`, functor-category abelian limits/colimits, stalks/germs, `constantSheaf`, sheafification, sheaf colimits, and stalk preservation of monos already own U001/U003/U007/U008/U011/U013/U017. `Spec.structureSheaf` owns the sheaf construction in U005 modulo the already-recorded classical-variety/structure-sheaf comparison. Function-sheaf reuse is stronger than the old audit recorded: pinned Mathlib has `TopCat.sheafToTop`, `StructureGroupoid.LocalInvariantProp.sheaf`, `smoothSheaf`, and `smoothSheafCommRing`; only a dedicated holomorphic-function specialization was not found in pinned/current Mathlib.
- 2026-09-13 remap-strict-bundle, affine/projective/basic-dimension block: U032–U035 now use the pinned field/DVR/Nullstellensatz/UFD spectrum APIs clause-by-clause; U044 records `Proj` of the standard polynomial grading as Mathlib-owned and leaves only the already-known classical `Projectivization` closed-point bridge; U064 uses `IdealSheafData.vanishingIdeal` plus `subscheme`, whose source documentation explicitly identifies the reduced induced structure; U065 uses `topologicalKrullDim`/`Order.coheight` and the affine height/coheight/stalk-dimension comparison theorems. No local project wrapper is treated as an owner for these standard objects.
- 2026-09-13 remap-strict-bundle, fibre/base-change definitions: U069 is the pullback over the terminal object `specZIsTerminal`; U072 is direct assembly of `Scheme.Hom.fiber`, connectedness, a rational point and an isomorphism; U073 uses the affine generic point/residue-field APIs together with `Rat.isFractionRing` and `Int.quotientSpanNatEquivZMod`; U075 uses the pinned stable-under-base-change instance for finite type and Mathlib's explicit `GeometricallyIntegral` strengthening to record why ordinary integrality is a different, non-geometric condition. The concrete failure examples remain U076–U077.
- 2026-09-13 remap-strict-bundle, separatedness/valuation block: U082 closes by combining the universal `IsImmersion (pullback.diagonal f)` instance with `IsClosedImmersion.iff_isPreimmersion`; U084 now records the much stronger `SpecToEquivOfLocalRing` classification and leaves only Hartshorne's reduced-closure/domination reformulation as comparison; U085 is exactly `Scheme.Hom.isClosedMap_iff_specializingMap` for quasi-compact morphisms.
