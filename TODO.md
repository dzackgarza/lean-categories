...
| `fc05-mapping` | **Completed 2026-09-14 after owner correction.** Reopened all unmatched FC05 definitions for live discovery regardless of stale `[negative-search-complete]`, ran source-limited OpenAPI bulk primary and mapping-hint censuses over all 49 rows, and re-adjudicated every row against source clauses plus targeted current/reference searches. FC05 now has 368/368 definition mappings accepted: 229 `mathlib`, 86 `project-existing`, 4 `reference-port`, and 49 freshly scoped `unmatched [negative-search-complete]`; the complete 1,059-unit FC05 catalogue also has 1,059/1,059 canonical mapping rows. The Mapping ledger is closed, and regeneration returns the 49 admissible unmatched rows to `author` only after that closure. | `definition-discovery` |
