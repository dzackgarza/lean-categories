...
| `definitions` | [Sweep III: definitions and intrinsic laws](#3-sweep-iii--realize-the-complete-definitional-layer) | `mapping`, `mapping-record-coverage`, `remap-strict-bundle` |
...
| `mapping-record-coverage` | **Open.** The canonical frontier currently finds 67 catalogue units with no parsed mapping row despite previously checked Mapping boxes: 3 in FC09 and 64 in FC13. A completed Mapping box means every admitted source unit has an explicit route result; absence is not `unmatched` and cannot authorize authorship. Reconcile the FC09 and FC13 catalogue IDs against their canonical mapping records, adding or repairing the missing rows with the normal exhaustive reuse search and without inventing a second residue ledger. The search must include the live [`formalization-corpus`](https://github.com/dzackgarza/formalization-corpus) API (`POST https://formalization-corpus.dzackgarza.com/api/search`; [query documentation](https://dzackgarza.github.io/formalization-corpus/api.html)) with multiple source/synonym/type-shape queries before any new `unmatched` verdict. Use its [`SOURCES.md`](https://github.com/dzackgarza/formalization-corpus/blob/main/SOURCES.md) for registry/provenance discovery, then pin actual upstream commits/paths in the mapping row. Run `just foundational-frontier` after each repair so the tracked `FOUNDATIONAL_FRONTIER.md` is regenerated from the canonical records. **Acceptance:** the frontier reports zero catalogue units lacking parsed mapping rows for every source whose Mapping box is checked; FC09 and FC13 are then checked again from that evidence. | `track-the-catalogue` |
| `remap-strict-bundle` | **Blocks every definition row below.** *Scope, corrected 2026-09-13:* count the route cell, not the justification text. `| \`unmatched\` |` appears on **6563** rows of 8888 route-bearing rows — 74% of the corpus — against **1795** on `mathlib` and **195** on `project-existing`. The 1305 rows carrying the strict-bundle wording are one fifth of the unmatched population, not the whole of it; the rest give other reasons and license invention identically. Every row-count previously written into this node was measured by grepping free text and was wrong in both directions. Report the **fraction**, not the raw count: matching a bundled row clause by clause correctly produces more rows, so the denominator grows during a good pass and the unmatched count can tick upward while real progress is being made. Between 06:29 and 12:30 on 2026-09-13 total route-bearing rows went 8888 to 9066, unmatched 6563 to 6409, and the fraction 73.8% to 70.7%; the `mathlib` count, 1795 to 2127, is monotone and is the other honest measure. The mapping sweep of 2026-09-06 wrote `unmatched` on rows whose clauses Mathlib supplies individually, justified as "strict bundle semantics reject … one clause/example of a bundled row, or a proper subset". That is not the rule [AGENTS.md](AGENTS.md) states, and `unmatched` is what clears a unit for repo-local invention — so the corpus has been authored where it should have been mapped. The justification appears on 240 of 325 rows in `chapter-2-schemes-fc06.md`, 183 of 188 in `chapter-4-homotopy-theory-fc07.md`, and 70 of 70 in `chapter-5-submanifolds-fc08.md`. Re-map every row carrying it, clause by clause, exhausting pinned Mathlib, current upstream, open PRs, the live [`formalization-corpus`](https://github.com/dzackgarza/formalization-corpus) search API, Reservoir, discoverable Lean repositories and the atlas before any row keeps `unmatched`; query the hosted corpus under multiple source names/synonyms/type fragments, then inspect and pin the actual upstream declaration rather than citing the index itself. Record the declarations supplying each clause and the comparison assembling them. Verified starting points: `FC06-C01-U013` and `FC06-C01-U015` are `MvPolynomial.vanishingIdeal_zeroLocus_eq_radical` and `zeroLocus_vanishingIdeal_galoisConnection`; `FC06-C01-U004` and `FC06-C01-U008` are the `zeroLocus` and irreducibility lemmas in `Spectrum/Prime/Basic.lean` and `Spectrum/Prime/Topology.lean`. Work source by source and finish both channels before moving on. A source's definition row unblocks only when *all* of its rows are re-mapped, `unmatched` and `project-existing` alike, so a pass that sweeps one channel horizontally across every source unblocks nothing: on 2026-09-13 this repository spent an hour at 37 commits per twenty minutes clearing `project-existing` rows from 465 to 375 while touching not one `unmatched` row, and no definition row moved any closer to ready. Take a source, clear its `project-existing` rows and its strict-bundle rows together, close it, and only then take the next — and **order the sources by actual `unmatched` route count, heaviest first** — count `| `unmatched` |`, never the strict-bundle wording, which is residue left in rows whose route has since moved and which points at the wrong files entirely: `chapter-2-schemes-fc06.md` carries 203 of that text and **3** unmatched rows. The real order is `chapter-4-homotopy-theory-fc07.md` 183, `chapter-3-cohomology-fc06.md` 181, `chapter-7-zeta-functions-and-l-series-fc09.md` 147, `chapter-3-cohomology-fc07.md` 147, `chapter-4-curves-fc06.md` 146, `chapter-5-surfaces-fc06.md` 144, `chapter-3-riemann-roch-theory-fc09.md` 118, `chapter-1-symmetric-and-quadratic-forms-an-overview-fc11.md` 106. Recompute that list rather than trusting these figures once they age; the command is one `grep -c` per file. Cover the second channel in the same pass: **466 rows carry `project-existing`**, and that label only preserves the audit claim when the declaration it names carries its own route. It frequently does not — `LeanCategories/Algebra/DimensionTheory.lean` is cited as the route by 6 rows while being the subject of 5 `unmatched` rows, so six units are audited against a file this catalogue says nothing realizes. The targets concentrate, which makes this cheap: `Modules/Bilinear/Valued/Fixed.lean` carries 13 rows, `Algebra/Concrete/Rings.lean` 9, `Homological/SpectralSequenceConvergence.lean` 8. Work target by target rather than row by row, and for each either find the library route the rows should have had, or map the local declaration itself so the rows inherit a real one. **Acceptance:** no row carries the strict-bundle justification; every `project-existing` row names a declaration that itself has a route; and each re-mapped row names its declarations and its comparison, committed in the tracked `corpus/` record | `track-the-catalogue`, `mapping`, `mapping-record-coverage` |
