...
| `fc05-mapping` | **Reopened after owner correction:** reference discovery is the objective; the prior FC05 negative-search completions are not accepted merely because their rows carried `[negative-search-complete]`. Re-adjudicate every currently unmatched FC05 definition from the corpus-wide candidate table and the full required discovery surface, retaining prior candidate/rejection evidence but actively looking for a reusable or reference implementation before greenfield work. Reuse every delivered positive route. **Acceptance:** every FC05 definition has a positive route or a newly revalidated `unmatched` row marked `[negative-search-complete]`; no FC05 greenfield Definition work resumes before this source-level mapping gate closes. | `definition-discovery` |
