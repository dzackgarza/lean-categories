...
| `retire-the-stale-catalogue-copy` | The migration under `track-the-catalogue` left a second, now-stale copy of every record reachable through the live `.agents/` symlink into the agent-memory vault, and the two have already diverged: `chapter-1-varieties-fc06.md` carries 40 strict-bundle rows under `corpus/` and 48 in the vault, with 65 files dirty there. Ignoring the symlink stops git seeing it; it does not stop a tool, a script or a worker reading it and getting mapping decisions that are hours out of date, and a stale mapping is the exact failure that sent this corpus into reinvention. Repoint or remove the symlink so the stale tree cannot be read from this repository, and confirm nothing in `scripts/` or the justfile still resolves through it. The vault copy itself is the owner's, not this repository's, to delete — leave it in place and say in the commit message that it is superseded, so whoever prunes the vault knows which side is authoritative. **Acceptance:** no path inside this repository resolves to the vault tree, and `rg` over `scripts/` and the justfile finds no reference to `.agents` | `track-the-catalogue` |
