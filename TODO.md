...
Read the frontier. Likewise, do not report the current `unmatched` fraction as the final amount of repo-local invention while `remap-strict-bundle` is open: `unmatched` is the negative-search worklist being audited, not a settled forecast. Report direct reuse/reference-port/unmatched separately and state that the latter can only authorize authorship after its source's remap has closed.
...
| `mapping-record-coverage` | **Completed 2026-09-13.** Every FC09 and FC13 catalogue unit now has a parsed mapping row; the canonical parser reports zero missing units for both sources, and their Mapping boxes are checked again. The final FC13 Appendix-C repair maps alternating/exterior algebra directly to Mathlib, records open-PR/reference routes for base change and Koszul complexes, and leaves only exhaustively searched mathematical gaps as `unmatched`. **Acceptance:** zero catalogue units lack parsed mapping rows for every source whose Mapping box is checked | `track-the-catalogue` |
| `remap-strict-bundle` | **Umbrella exact-route audit; it no longer globally blocks Definitions.** *Scope, corrected 2026-09-13:* count the route cell, not the justification text. `| \`unmatched\` |` appears on **6563** rows of 8888 route-bearing rows — 74% of the corpus — against **1795** on `mathlib` and **195** on `project-existing`. The 1305 rows carrying the strict-bundle wording are one fifth of the unmatched population, not the whole of it; the rest give other reasons and license invention identically. Every row-count previously written into this node was measured by grepping free text and was wrong in both directions. Report the **fraction**, not the raw count: matching a bundled row clause by clause correctly produces more rows, so the denominator grows during a good pass and the unmatched count can tick upward while real progress is being made. Between 06:29 and 12:30 on 2026-09-13 total route-bearing rows went 8888 to 9066, unmatched 6563 to 6409, and the fraction 73.8% to 70.7%; the `mathlib` count, 1795 to 2127, is monotone and is the other honest measure. The mapping sweep of 2026-09-06 wrote `unmatched` on rows whose clauses Mathlib supplies individually, justified as "strict bundle semantics reject … one clause/example of a bundled row, or a proper subset". That is not the rule [AGENTS.md](AGENTS.md) states, and `unmatched` is what clears a unit for repo-local invention — so the corpus has been authored where it should have been mapped. The justification appears on 240 of 325 rows in `chapter-2-schemes-fc06.md`, 183 of 188 in `chapter-4-homotopy-theory-fc07.md`, and 70 of 70 in `chapter-5-submanifolds-fc08.md`. Re-map every row carrying it, clause by clause, exhausting pinned Mathlib, current upstream, open PRs, the live [`formalization-corpus`](https://github.com/dzackgarza/formalization-corpus) search API, Reservoir, discoverable Lean repositories and the atlas before any row keeps `unmatched`; query the hosted corpus under multiple source names/synonyms/type fragments, then inspect and pin the actual upstream declaration rather than citing the index itself. Record the declarations supplying each clause and the comparison assembling them. Verified starting points: `FC06-C01-U013` and `FC06-C01-U015` are `MvPolynomial.vanishingIdeal_zeroLocus_eq_radical` and `zeroLocus_vanishingIdeal_galoisConnection`; `FC06-C01-U004` and `FC06-C01-U008` are the `zeroLocus` and irreducibility lemmas in `Spectrum/Prime/Basic.lean` and `Spectrum/Prime/Topology.lean`. Work source by source and finish both channels before moving on. A source's Definitions work unblocks when *that source* has closed both channels, `unmatched` and `project-existing`; it does **not** wait for the corpus-wide remap to finish. The former heaviest-unmatched-first global sweep was measured on 2026-09-14 after roughly eleven and a half hours of sustained remapping: despite high commit throughput it kept every Definitions node blocked and therefore serialized negative-search auditing in front of writing mathematics. That ordering is retired. The executable order is now the per-source pipeline below: close `fc05-remap`, do `fc05-definitions`, then close `fc06-remap`, do `fc06-definitions`, and so on. Already-delivered remaps for later sources remain valid prior work and shorten their later remap node; do not redo them. Cover the second channel in the same pass: **466 rows carry `project-existing`**, and that label only preserves the audit claim when the declaration it names carries its own route. It frequently does not — `LeanCategories/Algebra/DimensionTheory.lean` is cited as the route by 6 rows while being the subject of 5 `unmatched` rows, so six units are audited against a file this catalogue says nothing realizes. The targets concentrate, which makes this cheap: `Modules/Bilinear/Valued/Fixed.lean` carries 13 rows, `Algebra/Concrete/Rings.lean` 9, `Homological/SpectralSequenceConvergence.lean` 8. Work target by target rather than row by row, and for each either find the library route the rows should have had, or map the local declaration itself so the rows inherit a real one. **Acceptance:** no row carries the strict-bundle justification; every `project-existing` row names a declaration that itself has a route; and each re-mapped row names its declarations and its comparison, committed in the tracked `corpus/` record | `track-the-catalogue`, `mapping`, `mapping-record-coverage` |
| `fc05-remap` | Close the exact-route remap for FC05 only, immediately before FC05 Definitions. Reuse all already-delivered remap evidence for this source; do not re-search unchanged rows. **Acceptance:** every current source row has been checked under clause-wise route semantics; unsupported project-existing routes are repaired; any remaining unmatched residue has the repository-required negative-search evidence. This node gates only this source's Definitions | `mapping`, `mapping-record-coverage` |
| `fc06-remap` | Close the exact-route remap for FC06 only, immediately before FC06 Definitions. Reuse all already-delivered remap evidence for this source; do not re-search unchanged rows. **Acceptance:** every current source row has been checked under clause-wise route semantics; unsupported project-existing routes are repaired; any remaining unmatched residue has the repository-required negative-search evidence. This node gates only this source's Definitions | `fc05-definitions` |
| `fc07-remap` | Close the exact-route remap for FC07 only, immediately before FC07 Definitions. Reuse all already-delivered remap evidence for this source; do not re-search unchanged rows. **Acceptance:** every current source row has been checked under clause-wise route semantics; unsupported project-existing routes are repaired; any remaining unmatched residue has the repository-required negative-search evidence. This node gates only this source's Definitions | `fc06-definitions` |
| `fc08-remap` | Close the exact-route remap for FC08 only, immediately before FC08 Definitions. Reuse all already-delivered remap evidence for this source; do not re-search unchanged rows. **Acceptance:** every current source row has been checked under clause-wise route semantics; unsupported project-existing routes are repaired; any remaining unmatched residue has the repository-required negative-search evidence. This node gates only this source's Definitions | `fc07-definitions` |
| `fc09-remap` | Close the exact-route remap for FC09 only, immediately before FC09 Definitions. Reuse all already-delivered remap evidence for this source; do not re-search unchanged rows. **Acceptance:** every current source row has been checked under clause-wise route semantics; unsupported project-existing routes are repaired; any remaining unmatched residue has the repository-required negative-search evidence. This node gates only this source's Definitions | `fc08-definitions` |
| `fc10-remap` | Close the exact-route remap for FC10 only, immediately before FC10 Definitions. Reuse all already-delivered remap evidence for this source; do not re-search unchanged rows. **Acceptance:** every current source row has been checked under clause-wise route semantics; unsupported project-existing routes are repaired; any remaining unmatched residue has the repository-required negative-search evidence. This node gates only this source's Definitions | `fc09-definitions` |
| `fc11-remap` | **Completed 2026-09-13.** Close the exact-route remap for FC11 before its Definitions pass. The whole-source mapping record states that all 1,114 canonical units were rechecked clause by clause and records final route totals. **Acceptance:** every current FC11 mapping row has a checked route or supported unmatched residue, and every project-existing route is grounded at its owner | `fc10-definitions` |
| `fc12-remap` | Close the exact-route remap for FC12 only, immediately before FC12 Definitions. Reuse all already-delivered remap evidence for this source; do not re-search unchanged rows. **Acceptance:** every current source row has been checked under clause-wise route semantics; unsupported project-existing routes are repaired; any remaining unmatched residue has the repository-required negative-search evidence. This node gates only this source's Definitions | `fc11-definitions` |
| `fc13-remap` | Close the exact-route remap for FC13 only, immediately before FC13 Definitions. Reuse all already-delivered remap evidence for this source; do not re-search unchanged rows. **Acceptance:** every current source row has been checked under clause-wise route semantics; unsupported project-existing routes are repaired; any remaining unmatched residue has the repository-required negative-search evidence. This node gates only this source's Definitions | `fc12-definitions` |
| `fc14-remap` | Close the exact-route remap for FC14 only, immediately before FC14 Definitions. Reuse all already-delivered remap evidence for this source; do not re-search unchanged rows. **Acceptance:** every current source row has been checked under clause-wise route semantics; unsupported project-existing routes are repaired; any remaining unmatched residue has the repository-required negative-search evidence. This node gates only this source's Definitions | `fc13-definitions` |
| `fc15-remap` | Close the exact-route remap for FC15 only, immediately before FC15 Definitions. Reuse all already-delivered remap evidence for this source; do not re-search unchanged rows. **Acceptance:** every current source row has been checked under clause-wise route semantics; unsupported project-existing routes are repaired; any remaining unmatched residue has the repository-required negative-search evidence. This node gates only this source's Definitions | `fc14-definitions` |
| `fc16-remap` | Close the exact-route remap for FC16 only, immediately before FC16 Definitions. Reuse all already-delivered remap evidence for this source; do not re-search unchanged rows. **Acceptance:** every current source row has been checked under clause-wise route semantics; unsupported project-existing routes are repaired; any remaining unmatched residue has the repository-required negative-search evidence. This node gates only this source's Definitions | `fc15-definitions` |
...
| `fc05-definitions` | Sweep III for FC05: the pending count is the `## FC05 — Definitions` section of [FOUNDATIONAL_FRONTIER.md](FOUNDATIONAL_FRONTIER.md), which is regenerated; do not read a count from this row. Before adding new definitions, audit any existing definitions whose routes changed during this source remap; replace reinvention with the checked owner or record the mathematical rejection. Close this source's definitional layer before any FC05 theorem work; each definition carries its source citation and mapping record in the same commit | `fc05-remap` |
| `fc06-definitions` | Sweep III for FC06: the pending count is the `## FC06 — Definitions` section of [FOUNDATIONAL_FRONTIER.md](FOUNDATIONAL_FRONTIER.md), which is regenerated; do not read a count from this row. Before adding new definitions, audit any existing definitions whose routes changed during this source remap; replace reinvention with the checked owner or record the mathematical rejection. Close this source's definitional layer before any FC06 theorem work; each definition carries its source citation and mapping record in the same commit | `fc05-definitions`, `fc06-remap` |
| `fc07-definitions` | Sweep III for FC07: the pending count is the `## FC07 — Definitions` section of [FOUNDATIONAL_FRONTIER.md](FOUNDATIONAL_FRONTIER.md), which is regenerated; do not read a count from this row. Before adding new definitions, audit any existing definitions whose routes changed during this source remap; replace reinvention with the checked owner or record the mathematical rejection. Close this source's definitional layer before any FC07 theorem work; each definition carries its source citation and mapping record in the same commit | `fc06-definitions`, `fc07-remap` |
| `fc08-definitions` | Sweep III for FC08: the pending count is the `## FC08 — Definitions` section of [FOUNDATIONAL_FRONTIER.md](FOUNDATIONAL_FRONTIER.md), which is regenerated; do not read a count from this row. Before adding new definitions, audit any existing definitions whose routes changed during this source remap; replace reinvention with the checked owner or record the mathematical rejection. Close this source's definitional layer before any FC08 theorem work; each definition carries its source citation and mapping record in the same commit | `fc07-definitions`, `fc08-remap` |
| `fc09-definitions` | Sweep III for FC09: the pending count is the `## FC09 — Definitions` section of [FOUNDATIONAL_FRONTIER.md](FOUNDATIONAL_FRONTIER.md), which is regenerated; do not read a count from this row. Before adding new definitions, audit any existing definitions whose routes changed during this source remap; replace reinvention with the checked owner or record the mathematical rejection. Close this source's definitional layer before any FC09 theorem work; each definition carries its source citation and mapping record in the same commit | `fc08-definitions`, `fc09-remap` |
| `fc10-definitions` | Sweep III for FC10: the pending count is the `## FC10 — Definitions` section of [FOUNDATIONAL_FRONTIER.md](FOUNDATIONAL_FRONTIER.md), which is regenerated; do not read a count from this row. Before adding new definitions, audit any existing definitions whose routes changed during this source remap; replace reinvention with the checked owner or record the mathematical rejection. Close this source's definitional layer before any FC10 theorem work; each definition carries its source citation and mapping record in the same commit | `fc09-definitions`, `fc10-remap` |
| `fc11-definitions` | Sweep III for FC11: the pending count is the `## FC11 — Definitions` section of [FOUNDATIONAL_FRONTIER.md](FOUNDATIONAL_FRONTIER.md), which is regenerated; do not read a count from this row. Before adding new definitions, audit any existing definitions whose routes changed during this source remap; replace reinvention with the checked owner or record the mathematical rejection. Close this source's definitional layer before any FC11 theorem work; each definition carries its source citation and mapping record in the same commit | `fc10-definitions`, `fc11-remap` |
| `fc12-definitions` | Sweep III for FC12: the pending count is the `## FC12 — Definitions` section of [FOUNDATIONAL_FRONTIER.md](FOUNDATIONAL_FRONTIER.md), which is regenerated; do not read a count from this row. Before adding new definitions, audit any existing definitions whose routes changed during this source remap; replace reinvention with the checked owner or record the mathematical rejection. Close this source's definitional layer before any FC12 theorem work; each definition carries its source citation and mapping record in the same commit | `fc11-definitions`, `fc12-remap` |
| `fc13-definitions` | Sweep III for FC13: the pending count is the `## FC13 — Definitions` section of [FOUNDATIONAL_FRONTIER.md](FOUNDATIONAL_FRONTIER.md), which is regenerated; do not read a count from this row. Before adding new definitions, audit any existing definitions whose routes changed during this source remap; replace reinvention with the checked owner or record the mathematical rejection. Close this source's definitional layer before any FC13 theorem work; each definition carries its source citation and mapping record in the same commit | `fc12-definitions`, `fc13-remap` |
| `fc14-definitions` | Sweep III for FC14: the pending count is the `## FC14 — Definitions` section of [FOUNDATIONAL_FRONTIER.md](FOUNDATIONAL_FRONTIER.md), which is regenerated; do not read a count from this row. Before adding new definitions, audit any existing definitions whose routes changed during this source remap; replace reinvention with the checked owner or record the mathematical rejection. Close this source's definitional layer before any FC14 theorem work; each definition carries its source citation and mapping record in the same commit | `fc13-definitions`, `fc14-remap` |
| `fc15-definitions` | Sweep III for FC15: the pending count is the `## FC15 — Definitions` section of [FOUNDATIONAL_FRONTIER.md](FOUNDATIONAL_FRONTIER.md), which is regenerated; do not read a count from this row. Before adding new definitions, audit any existing definitions whose routes changed during this source remap; replace reinvention with the checked owner or record the mathematical rejection. Close this source's definitional layer before any FC15 theorem work; each definition carries its source citation and mapping record in the same commit | `fc14-definitions`, `fc15-remap` |
| `fc16-definitions` | Sweep III for FC16: the pending count is the `## FC16 — Definitions` section of [FOUNDATIONAL_FRONTIER.md](FOUNDATIONAL_FRONTIER.md), which is regenerated; do not read a count from this row. Before adding new definitions, audit any existing definitions whose routes changed during this source remap; replace reinvention with the checked owner or record the mathematical rejection. Close this source's definitional layer before any FC16 theorem work; each definition carries its source citation and mapping record in the same commit | `fc15-definitions`, `fc16-remap` |
